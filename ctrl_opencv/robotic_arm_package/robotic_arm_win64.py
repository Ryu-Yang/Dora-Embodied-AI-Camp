...
class Set_Tool_IO():
    ...
    def Set_Tool_Voltage(self, type, block=True):
        ...
        logger_.info(f'The result of Set_Tool_Voltage: {tag}')
        return tag
...
class Arm(Set_Joint, Get_Joint, Tcp_Config, Tool_Frame, Work_Frame, Arm_State, Initial_Pose, Move_Plan, Teaching,
    ...
    def __init__(self, dev_mode, ip, pCallback=None):
        ...
        logger_.info('Started initializing the robotic arm API. Completed')
        ...
        if state:
            logger_.info(f'Failed to connect the robotic arm: {self.nSocket}')

        else:
            logger_.info(f'Successfully connected the robotic arm, handle is: {self.nSocket}')
